...
class PinchZoomRecyclerView @JvmOverloads constructor(
    ...
    // Add this to the PinchZoomRecyclerView class
    fun resetZoom(animate: Boolean = true) {
        if (scaleFactor == 1f) return // Already at normal zoom

        if (animate) {
            // Smooth animation to reset zoom
            ValueAnimator.ofFloat(scaleFactor, 1f).apply {
                ...
                addUpdateListener { animator ->
                    ...
                    posY = 0f
                    invalidate()
                    zoomChangeListener?.invoke(false, scaleFactor)
                }
                start()
            }
        } else {
        ...
        // Reset scroll position to ensure proper alignment
        post {
